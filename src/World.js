...
export class World {
  ...
  autoLoadChunks() {
    ...
    this.fchunks = setInterval(function () {
      t.freeChunks();
    }, 100);
  }
  ...
  freeChunks() {
    ...
    this.chunks.forEach(function (chunk, key) {
      var dx = Math.floor(
        Math.abs(chunk.position[0] * t.cl + t.cl / 2 - t.player.eyePos[0]) /
          t.cl
      );
      var dz = Math.floor(
        Math.abs(chunk.position[2] * t.cl + t.cl / 2 - t.player.eyePos[2]) /
          t.cl
      );
  ...
  loader() {
    ...
    var ex = Math.floor(
      Math.min(this.size - 1, this.player.eyePos[0] / this.cl + p)
    );
    var ez = Math.floor(
      Math.min(this.size - 1, this.player.eyePos[2] / this.cl + p)
    );
  ...
  createBorders() {
    ...
    const face = [0, 1, 0, 1, 1, 0, 1, 0, 0, 0, 0, 0];
    ...
    for (var x = 0; x < this.worldLength; x++) {
      for (var y = 0; y < this.worldHeight; y++) {
        ...
        wall.indices[i * 6 + 0] = i * 4 + 2;
        wall.indices[i * 6 + 1] = i * 4 + 1;
        wall.indices[i * 6 + 2] = i * 4;
        wall.indices[i * 6 + 3] = i * 4 + 3;
        wall.indices[i * 6 + 4] = i * 4 + 2;
        wall.indices[i * 6 + 5] = i * 4;

        wall.coords[i * 8 + 0] = rc[0];
        wall.coords[i * 8 + 1] = rc[1];
        wall.coords[i * 8 + 2] = rc[2];
        wall.coords[i * 8 + 3] = rc[1];
        wall.coords[i * 8 + 4] = rc[2];
        wall.coords[i * 8 + 5] = rc[3];
        wall.coords[i * 8 + 6] = rc[0];
        wall.coords[i * 8 + 7] = rc[3];
      }
    }
  ...
  generateTerrain(update, finish) {
    ...
    generate(
      t.generateChunkTerrain.bind(t),
      "Generating chunk terrain.",
      function () {
        generate(
          t.generateChunkCaves.bind(t),
          "Generating caves.",
          function () {
            generate(
              t.decorateChunk.bind(t),
              "Decorating chunks.",
              function () {
                t.placePlayer();
                finish();
    ...
    for (var x = cx * this.cl; x < (cx + 1) * this.cl; x++) {
      for (var z = cz * this.cl; z < (cz + 1) * this.cl; z++) {
        for (var y = 0; y < this.worldHeight; y++) {
          var n3d =
            (y / 65 +
              noise.simplex3(x / 12, y / 10, z / 12) * 3 +
              noise.simplex3(x / 80, y / 20, z / 80) * 10) /
            14;
  ...
  getBlock(x, y, z) {
    ...
    return blocks[
      this.map[
        y * this.worldLength * this.worldLength + z * this.worldLength + x
      ]
    ];
  }
