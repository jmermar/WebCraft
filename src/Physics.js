  ...
  static collision(box, ignore) {
    ...
    for (var x = sx; x <= ex; x++) {
      for (var y = sy; y <= ey; y++) {
        for (var z = sz; z <= ez; z++) {
          if (
            global.world.getBlock(x, y, z).nocoll != true &&
            !ignore.has([x, y, z].join(","))
          ) {
            coll = true;
            ...
            b.add([x, y, z].join(","));
          } else if (global.world.getBlock(x, y, z).name == "water") {
            water = true;
          }
        }
      }
...
export class Raycast {
  ...
  getBlock(step) {
    ...
    for (var i = 0; i < this.max; i += step) {
      ...
      block = global.world.getBlock(bx, by, bz);
